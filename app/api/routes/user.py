...
def InivteUser(user_invite: UserInviteViewModel, background_tasks: BackgroundTasks,
    ...
    # 寄信
    token_data = send_invite_mail(user_invite.email, user_invite.level, current_user.group_id, background_tasks=background_tasks)
    with open(os.getcwd() + '/Pytest/Default/User_data/invite_token.txt','w') as token:
        token.write(token_data)
    return "Email has sended"
...
def CreateUser(user_create: UserPostViewModel, token: str,
    ...
    user_db = Create_User(db, user_create, data["group_id"], data["level"], is_enable=True)
    return user_db
