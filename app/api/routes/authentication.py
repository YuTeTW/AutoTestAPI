...
def login(user_data: UserLoginViewModel, background_tasks: BackgroundTasks,
    ...
    if user.verify_code_enable:
        ...
        # 寄信
        token_data = send_Verfiy_code_email_async(user_data.email, verify_code, background_tasks=background_tasks)
        with open(os.getcwd() + '/Pytest/Default/User_data/verify_code.txt', 'w') as token:
